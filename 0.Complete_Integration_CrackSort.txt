...
# This is the complete code for the identification and classfication of crack
...
# MODEL CONFIGURATION
##############################################################################
model_path = r"C:\Users\PMLS\Downloads\crack_segmentation_model (2).pth"
class_names = {
    0: "Background",
    1: "Compression Crack", 
    2: "Rebar Detachment",
...
##############################################################################
# GLOBAL VARIABLES & CONSTANTS (GUI + Segmentation)
##############################################################################
...
pred_mask = None  # segmentation mask prediction
zoom_factor = 5  
is_zoomed = False
mouse_x, mouse_y = 0, 0
distance_mode = False
...
##############################################################################
def predict_segmentation_mask(img):
    ...
    pred_mask_resized = cv2.resize(pred, (img.shape[1], img.shape[0]), interpolation=cv2.INTER_NEAREST)
...
def load_image():
    global image, crack_image_on_bright, pred_mask, polygon_points, polygon_mode
...
def reset_main_image(new_img):
    global image
    image = new_img.copy()
    # Process image for scaling factors and crack detection overlay
    process_image()
    # Predict segmentation mask for crack type detection
    global pred_mask
    pred_mask = predict_segmentation_mask(image)
...
def detect_cracks_no_area(image_no_grid):
    ...
    brightened_image = cv2.convertScaleAbs(gray_image, alpha=0.78, beta=12)
...
def compute_cell_area(x, y):
    global crack_image_on_bright, grid_spacing_mm, scaling_factor_width, scaling_factor_height, highlighted_cell, pred_mask
    ...
    # Also determine crack type using segmentation mask if available
    if pred_mask is not None:
        cell_mask = pred_mask[y1:y2, x1:x2]
        unique = np.unique(cell_mask)
        crack_types = [class_names.get(cid, "Unknown") for cid in unique if cid != 0]
        if crack_types:
            print("  Crack Types Detected:")
            for ct in crack_types:
                print(f"   - {ct}")
        else:
            print("  No crack types detected.")
...
def main_loop():
    ...
    global polygon_mode, polygon_points, pred_mask
    ...
    while True:
        ...
        if pred_mask is not None:
            ...
            base_display = cv2.addWeighted(base_display, 0.8, color_mask, 0.2, 0)
